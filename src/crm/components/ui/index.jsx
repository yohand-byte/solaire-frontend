  ...
  return (
    <div className="space-y-1">
      ...
      <div className="relative">
        <select
          className={clsx(
            'block w-full rounded-xl border-gray-200 shadow-sm transition-all duration-200 appearance-none',
            'focus:border-primary-500 focus:ring-primary-500 focus:ring-2 focus:ring-opacity-20',
            'pr-10',
            className
          )}
...
export function WorkflowStep({ label, status, icon: Icon }) {
  const statusConfig = {
    pending: { color: 'bg-gray-200 text-gray-500', icon: '○' },
    in_progress: { color: 'bg-amber-100 text-amber-600', icon: '◐' },
    completed: { color: 'bg-emerald-100 text-emerald-600', icon: '✓' },
    blocked: { color: 'bg-red-100 text-red-600', icon: '✕' },
  };
